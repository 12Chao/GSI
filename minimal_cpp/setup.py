...
def make_extension():
    parent_p = "/global/cfs/cdirs/ntrain4/comocheng/gpuhackathon2023/chao/GSIpy/*"
    sources = sorted(glob(join(parent_p, "*.cpp"))) + \
              sorted(glob(join(parent_p, "*.cu")))

    includes = [os.path.dirname(i) for i in sorted(glob(join(parent_p, "*.h")))] + [pybind11.get_include(True), pybind11.get_include(False)]
